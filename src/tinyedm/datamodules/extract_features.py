...
class ImageFeatureExtractor(L.LightningModule):
    ...
    def on_predict_start(self):
        self.mean = torch.tensor([5.81,3.25,0.12,-2.15], device=self.device).view(1, -1, 1, 1)
        self.std = torch.tensor([4.17,4.62,3.71,3.28], device=self.device).view(1, -1, 1, 1) * 2
    ...
    def predict_dataloader(self):
        transform = transforms.Compose([
            transforms.Lambda(lambda pil_image: center_crop_arr(pil_image, self.args.image_size)),
...
def main(args):
    ...
    feature_writer = FeaturePredictionWriter(feature_dir, label_dir, write_interval='batch')
    trainer = L.Trainer(
        accelerator="gpu",
        precision=32,
        callbacks=[feature_writer]
    )
